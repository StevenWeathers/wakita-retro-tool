...
// handleGetTeamRetrospectives gets a list of retrospectives associated to the team
func (s *server) handleGetTeamRetrospectives() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		vars := mux.Vars(r)
		TeamID := vars["teamId"]
		Limit, _ := strconv.Atoi(vars["limit"])
		...
		Retrospectives := s.database.TeamRetrospectiveList(TeamID, Limit, Offset)

		s.respondWithJSON(w, http.StatusOK, Retrospectives)
	}
}

// handleTeamRemoveRetrospective handles removing retrospective from a team
func (s *server) handleTeamRemoveRetrospective() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		keyVal := s.getJSONRequestBody(r, w)
		...
		err := s.database.TeamRemoveRetrospective(TeamID, RetrospectiveID)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
